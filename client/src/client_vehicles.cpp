...
namespace vehicles {
  ...
  Vehicle load_vehicle_from_file(const string& file_name) {
    ...
    if (!steering_values.IsDefined()) log_error(file_name + ": missing node \"steering\"");
    ...
    try {
      ...
      has_errors = true;
      if (error.mark.is_null()) log_error(file_name + ": missing parameter \"inertia\"");
      else log_error(file_name + ": invalid vehicle parameter \"inertia\"");
    }
    ...
    try {
      ...
      has_errors = true;
      if (error.mark.is_null()) log_error(file_name + ": missing parameter \"friction_torque\"");
      else log_error(file_name + ": invalid vehicle parameter \"friction_torque\"");
    }
    ...
    try {
      ...
      has_errors = true;
      if (error.mark.is_null()) log_error(file_name + ": missing parameter \"wheel_radius\"");
      else log_error(file_name + ": invalid vehicle parameter \"wheel_radius\"");
    }
    ...
    try {
      ...
      has_errors = true;
      if (error.mark.is_null()) log_error(file_name + ": missing parameter \"aerodynamic_drag\"");
      else log_error(file_name + ": invalid vehicle parameter \"aerodynamic_drag\"");
    }
    ...
    try {
      ...
      has_errors = true;
      if (error.mark.is_null()) log_error(file_name + ": missing parameter \"rolling_resistance\"");
      else log_error(file_name + ": invalid vehicle parameter \"rolling_resistance\"");
    }
    ...
    try {
      ...
      has_errors = true;
      if (error.mark.is_null()) log_error(file_name + ": missing parameter \"braking_power\"");
      else log_error(file_name + ": invalid vehicle parameter \"braking_power\"");
    }
    ...
    try {
      ...
      has_errors = true;
      if (error.mark.is_null()) log_error(file_name + ": missing parameter \"lateral_stiffness\"");
      else log_error(file_name + ": invalid vehicle parameter \"lateral_stiffness\"");
    }
    ...
    try {
      ...
      has_errors = true;
      if (error.mark.is_null()) log_error(file_name + ": missing parameter \"wheelbase\"");
      else log_error(file_name + ": invalid vehicle parameter \"wheelbase\"");
    }
    ...
    try {
      ...
      has_errors = true;
      if (error.mark.is_null()) log_error(file_name + ": missing parameter \"sensitivity\"");
      else log_error(file_name + ": invalid vehicle parameter \"sensitivity\"");
    }
    ...
    try {
      ...
      has_errors = true;
      if (error.mark.is_null()) log_error(file_name + ": missing parameter \"responsiveness\"");
      else log_error(file_name + ": invalid vehicle parameter \"responsiveness\"");
    }
