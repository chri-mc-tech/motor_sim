...
namespace physics {
  void physics_loop() {
    ...
    main_vehicle.current_engine_torque =
        main_vehicle.max_engine_torque * (1 - pow(((main_vehicle.current_engine_rpm - peak_rpm) / peak_rpm), 2));
    ...
    if (main_vehicle.current_engine_rpm >= main_vehicle.max_rpm) {
      ...
      main_vehicle.current_engine_rpm =
          std::clamp(main_vehicle.current_engine_rpm, main_vehicle.idle_rpm, main_vehicle.max_rpm);
    }
    ...
    main_vehicle.current_wheels_torque = main_vehicle.current_engine_torque * main_vehicle.current_throttle *
                                         main_vehicle.current_gear_ratio * main_vehicle.final_drive;
    ...
    double downforce = main_vehicle.downforce_factor * main_vehicle.current_forward_velocity *
                       abs(main_vehicle.current_forward_velocity);
    ...
    double air_resistance = main_vehicle.aerodynamic_drag * main_vehicle.current_forward_velocity *
                            abs(main_vehicle.current_forward_velocity);
    double rolling_resistance = main_vehicle.rolling_resistance * main_vehicle.current_forward_velocity;

    int temp;
    if (main_vehicle.current_forward_velocity == 0) {
      temp = 0;
    } else if (main_vehicle.current_forward_velocity > 0) {
      temp = 1;
    } else if (main_vehicle.current_forward_velocity < 0) {
      temp = -1;
    }
    ...
    if (main_vehicle.current_throttle == 0 && main_vehicle.current_gear != 0) {
      double braking_torque_at_wheels =
          main_vehicle.engine_friction_torque * abs(main_vehicle.current_gear_ratio) * main_vehicle.final_drive;

      engine_braking = (braking_torque_at_wheels / main_vehicle.wheel_radius) * temp;
    }

    main_vehicle.current_wheels_force =
        main_vehicle.current_wheels_force - air_resistance - rolling_resistance - brake_force - engine_braking;
    ...
    if (main_vehicle.current_gear == 0) {
      double engine_net_torque =
          (main_vehicle.current_engine_torque * main_vehicle.current_throttle) - main_vehicle.engine_friction_torque;
      ...
      main_vehicle.current_engine_rpm = abs(wheels_angular_velocity) * main_vehicle.current_gear_ratio *
                                        main_vehicle.final_drive * (60.0 / (2.0 * std::numbers::pi));
    }

    main_vehicle.current_engine_rpm =
        std::clamp(main_vehicle.current_engine_rpm, main_vehicle.idle_rpm, main_vehicle.max_rpm);

    double target_lat_vel =
        -main_vehicle.current_steer * abs(main_vehicle.current_forward_velocity) * main_vehicle.steer_sensitivity;

    main_vehicle.current_lateral_velocity +=
        (target_lat_vel - main_vehicle.current_lateral_velocity) * main_vehicle.steering_responsiveness * delta_time;

    double max_lat_limit = main_vehicle.grip * physics::GRAVITATIONAL_FORCE * (main_vehicle.lateral_stiffness / 6.0);
    main_vehicle.current_lateral_velocity =
        std::clamp(main_vehicle.current_lateral_velocity, -max_lat_limit, max_lat_limit);
    ...
    main_vehicle.current_location.x += (main_vehicle.current_forward.x * main_vehicle.current_forward_velocity +
                                        main_vehicle.current_right.x * main_vehicle.current_lateral_velocity) *
                                       delta_time;
    main_vehicle.current_location.z += (main_vehicle.current_forward.z * main_vehicle.current_forward_velocity +
                                        main_vehicle.current_right.z * main_vehicle.current_lateral_velocity) *
                                       delta_time;
